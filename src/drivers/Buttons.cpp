...
bool Buttons::upPressed() { return btnUp.rose() && !upHoldFired; }

bool Buttons::downPressed() { return btnDown.rose() && !downHoldFired; }

bool Buttons::selectPressed() { return btnSelect.rose() && !selectHoldFired; }

bool Buttons::upHeld(uint16_t holdMs) {
  if (!upHoldFired && btnUp.read() == LOW && millis() - upPressedAt >= holdMs) {
...
bool Buttons::downHeld(uint16_t holdMs) {
  if (!downHoldFired && btnDown.read() == LOW &&
...
bool Buttons::selectHeld(uint16_t holdMs) {
  if (!selectHoldFired && btnSelect.read() == LOW &&
